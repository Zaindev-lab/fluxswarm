...
def _cmd_dispatch(args: argparse.Namespace) -> int:
    ...
    # Resolve the board slug so dispatch concurrency accounting (which is
    # host-level and hops across boards) is anchored to THIS board rather
    # than defaulting to None and counting stale 'running' tasks on every
    # OTHER board against this board's budget. The gateway dispatcher
    # threads `board=slug` through; mirror it here so the CLI path behaves
    # identically and a single-board deployment (FluxSwarm launches one
    # board per swarm) is never starved by unrelated boards.
    try:
        _dispatch_board = kb.get_current_board()
    except Exception:
        _dispatch_board = None
    with kb.connect_closing() as conn:
        res = kb.dispatch_once(
            conn,
            board=_dispatch_board,
